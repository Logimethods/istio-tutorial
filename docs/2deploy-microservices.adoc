...
docker push logimethods/istio-customer
...
mvn clean package
docker build -t logimethods/istio-preference:v1 .
docker push logimethods/istio-preference:v1
...
----
mvn clean package
docker build -t logimethods/istio-{servicename}:v1 .
